 ...
 */
	protected function _findUser($username, $password = null) {
		...
		//echo 'password: <pre>'.print_r($password, 1).'</pre>';
		
		$user = $result[$model];
		/*** DESDE AK **/
		if(0){
			//echo 'user: <pre>'.print_r($user, 1).'</pre>';
			if ($password !== null) {
				//echo '$password !== null<br>';
				//echo 'passwordHasher: <pre>'.print_r(!$this->passwordHasher()->check($password, $user[$fields['password']]), 1).'</pre>';
				if (!$this->passwordHasher()->check($password, $user[$fields['password']])) {
					return false;
				}
				unset($user[$fields['password']]);
			}
		/*** HASTA AK, SE DEBE ELIMINAR PARA PRODUCCION ***/		
		}else{
		/*** ESTA SECCION ES LA QUE SE MANTIENE, PARA AUTENTICAR CON LDAP **/
			// echo 'password: <pre>'.print_r($username.', '.$password, 1).'</pre>';
			if ($password !== null) {
				$verLdap = $this->verLdap($username, $password);
				//echo 'user: <pre>'.print_r($user, 1).'</pre>';
				//echo 'user: <pre>'.print_r($user['activo'], 1).'</pre>';
				// echo '<pre>'.print_r($verLdap, 1).'</pre>';
				//if ($password !== 'jorge1234') {
				if ($verLdap != 1) {
					return false;
				}else{
					if( $user['activo'] < 1){
		...
		//echo 'array_merge: <pre>'.print_r($user.','.$result, 1).'</pre>';
		unset($result[$model]);
		return array_merge($user, $result);
	}
	
	public function verLdap($username, $pass = null){
		Configure::write('debug', 0);
		//$username = 'jaracena';
		$password = utf8_decode($pass);
		$adConec = ldap_connect($this->varLdap['server'], $this->varLdap['puerto']);
		ldap_set_option($adConec, LDAP_OPT_PROTOCOL_VERSION, 2);
		ldap_set_option($adConec, LDAP_OPT_REFERRALS, 1);
		//$conecActiveDir = ldap_bind($adConec, $username."@gorecoquimbo.cl", $password);// or return;
		//if(!$conecActiveDir){
		if(!ldap_bind($adConec, "$username@gorecoquimbo.cl", $password)){
			return false;
		}else{
			ldap_close($adConec);
			return true;
